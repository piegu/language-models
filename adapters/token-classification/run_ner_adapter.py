...
[07/19/2021] Added functions by Pierre GUILLOU
into the script https://github.com/Adapter-Hub/adapter-transformers/blob/master/examples/token-classification/run_ner.py:
...
Link to this file in github: https://github.com/piegu/language-models/blob/master/adapters/token-classification/run_ner.py
"""
...
class DataTrainingArguments:
    ...
    early_stopping_patience: int = field(
        default=1,
        metadata={
            "help": "early_stopping_patience: number of epochs to do before to stop the training."
        },
    )
    # new 
    madx2: bool = field(
        default=False,
        metadata={
            "help": "MAD-X 2.0: there is no adapter in the last transformer layer if True."
        },
    )
    # new 
    adapter_composition: str = field(
        default="stack",
        metadata={
    ...
    houlsby_MHA_lastlayer: bool = field(
        default=False,
        metadata={
            "help": "Put only the adapter after the MHA but not after the FeedForward in the last layer."
        },
    )
...
#             )
    ...
    if data_args.houlsby_MHA_lastlayer and adapter_args.train_adapter and not data_args.madx2 and task_name in model.config.adapters and (adapter_config_name == "houlsby" or adapter_config_name == "houlsby+inv"):
        import torch
    ...
    trainer = Trainer(
        ...
        callbacks=[EarlyStoppingCallback(early_stopping_patience=data_args.early_stopping_patience)] if training_args.load_best_model_at_end else None,
    )
